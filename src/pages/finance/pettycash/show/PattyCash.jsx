...
import { fakepatty } from "../../../../components/FakeData";

const PattyCash = () => {
  const [dataList, setDataList] = useState(fakepatty);
  ...
  /* =========================
     TABLE DATA (ONLY id, entryNo, date)
  ========================= */
  ...
  const handleSaveInvoice = () => {
    if (!invoiceData.entryNo) {
      Swal.fire("Error", "Entry No required", "error");
      return;
    }
    ...
    setInvoiceData({});
    setResetKey((k) => k + 1);
    Swal.fire("Success", "Petty cash added", "success");
  };
  ...
  const handleUpdateInvoice = () => {
    setDataList((prev) =>
      prev.map((i) => (i.id === editInvoiceData.id ? editInvoiceData : i))
    );
  ...
  return (
    <div className="space-y-3 w-full">
      ...
      <div className="flex justify-end gap-3">
        <DownloadDataButton data={tableData} fileName="Petty Cash" />
      ...
      <InvoiceModal
        ...
        content={
          <UpdateInvoice
            ...
            onUpdate={setEditInvoiceData}     // pass setter
          />
        }
      />
    </div>
