...
import { fakePurchaseGrn } from "../../../../components/FakeData";
...
const PurchaseGrn = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterType === "active") {
      updated = updated.filter((i) => i.invoice.status === "Active");
    }
  ...
  const tableData = useMemo(() => {
    return filteredData.map((item) => {
      const totalProducts = item.products.reduce((sum, p) => sum + p.poQty, 0);

      return {
        id: item.id,
        grnNo: item.invoice.grnNo,
        vehicleNo: item.invoice.vehicleNo,
        poNo: item.invoice.poNo,
        receivedBy: item.invoice.receivedBy,
  ...
  const handleSaveInvoice = () => {
    if (!invoiceData.grnNo || products.length === 0) {
    ...
    Swal.fire("Success", "Purchase GRN added successfully", "success");
